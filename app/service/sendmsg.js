...
class sendmsg extends Service {
  ...
  async getWeather(city = '上海') {
    const { app } = this;
    const url = 'http://www.tianqiapi.com/free/day?appid=' + app.config.weather.appid + '&appsecret=' + app.config.weather.appsecret + '&unescape=1';
  ...
  getMarryDay() {
    ...
    console.log(moment(moment(resultMarry).format()).diff(moment(now).format(), 'day'), 'loveDay11');
  ...
  getbirthday() {
    ...
    console.log(Math.floor(resultBirthday / (1000 * 3600 * 24)), 'resultBirthday');
    return Math.floor(resultBirthday / (100000 * 3600 * 24));
  }
  // 获取 相恋天数
  getLoveDay() {
    const { app } = this;
